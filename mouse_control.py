        ...
        for pos in positions:
            if not stop_flag.is_set():
                if isinstance(pos, list) and len(pos) == 2:
                    ...
                    time.sleep(0.05)  # Небольшая задержка перед отпусканием клавиши
                    keyboard_controller.release('a')
                    
                    time.sleep(0.2)  # Задержка между кликами для наглядности
...
def start_recording():
    ...
    with open('location_new.json', 'w') as file:
        json.dump({'pos': click_positions}, file)
    print("Clicks saved to location_new.json")

# Функция для отслеживания горячих клавиш
def listen_for_hotkeys():
    # Воспроизведение последовательных кликов из файла location1.json
    keyboard.add_hotkey('ctrl+alt+s', lambda: sequential_clicks(load_positions('location1.json')))
    ...
    keyboard.add_hotkey('ctrl+n', lambda: sequential_clicks(load_positions('location_new.json')))
